...
| Players | 5-character usernames with **on-brand mutation**: type 5 alphanumerics ("h3nry"), press Enter, and one character is swapped for one of the 32 keyboard symbols ("h3n$y"); "FIX MY USERNAME" just re-mutates a *different* character; starting the game makes it permanent (`ttf.user`). Shown in HUD and results. **No game-save by design** — too fast-paced; only identity + sound settings persist |
